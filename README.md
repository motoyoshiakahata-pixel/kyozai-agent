...
スキルの答案様式は、教員が過去に手作業で作成してきた「問題モデル」（`12_問題モデルフォルダ`）の実物に厳密に合わせています（導入文の書き出し・Sさん/Tさんのような連続する記号名・空欄記号の通し方・①〜⑥の6択で全空欄とX/Yの正誤を1行にまとめる・リード文中への出典ページの明記・問題編と解答解説編を別ファイルに分ける、など）。パターンB（資料読み取り型）は、意見文ア〜ウの組合せを選ばせる形式（B-2）ではなく、統計の数値そのものを空欄にしてパターンAに統合する形式（B-1）を既定にしています。これは実際の問題モデルがすべてこの形式だったためです。様式を変更したいときは `.claude/skills/kyozai/SKILL.md` の「答案の様式」「見本」節を編集してください。

#### 単元番号だけで手早く依頼する

毎回「単元名（教科書p.○○-△△）」と書かなくても済むよう、`/kyozai` コマンド（`.claude/commands/kyozai.md`）と単元一覧（`.claude/skills/kyozai/units.md`）を用意しています。`.claude/skills/kyozai/units.md` には、教員が実際に授業で使っている単元番号・単元名・ページ範囲の対応表（教員のGoogleドライブの「07_小テスト」フォルダの命名から作成）が入っており、`/kyozai ② 3問` のように番号だけで依頼できます。単元名やページを明示すればそちらが優先されます。新しい単元で小テストを作った場合は、`units.md` に行を追加してください（フォルダ名からそのまま転記できます）。
