...
`.claude/commands/kyozai.md` により、`/kyozai 「他者とともに生きる人間としてのあり方」（教科書p.14-21）で3問` のように直接依頼を書けます。単元名・ページ範囲は毎回の依頼で明示してください（教員のGoogleドライブの「07_小テスト」フォルダは教員本人の作業スペースのため、単元名の情報源としては参照しません）。単元名やページ範囲が書かれていない依頼を受けたときは、作成プランを立てる前に確認するようスキルに指示しています。
